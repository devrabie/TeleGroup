...
from src.plugins.common import aliases, message_text, missing_dependency, tr
...
class SpeechPlugin(Plugin):
    ...
    async def handle(self, ctx: CommandContext) -> None:
        ...
        if parsed is None:
            await ctx.reply(
                tr(ctx, "Send text to speak, or reply to it.", "أرسل نصاً لنطقه أو رد عليه.")
            )
            return
        lang, text = parsed
        status = await ctx.reply(tr(ctx, "Speaking…", "جارٍ النطق…"))
            ...
            if not result.get("ok"):
                if result.get("error") == "missing":
                    ...
                    await edit_status(
                        ctx,
                        status,
                        tr(ctx, "That language code is not supported.", "رمز اللغة غير مدعوم."),
                    )
                else:
                    await edit_status(
                        ctx, status, tr(ctx, "Could not speak that text.", "تعذر نطق النص.")
                    )
                return
