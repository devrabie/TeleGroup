...
Userbot commands use `USERBOT_PREFIX` (default `.`). The account itself can send them, and so can the registered owner from their own Telegram account (private chat or a shared group). The owner can add more people with `.رفع ادمن` / `.addadmin` (by reply, numeric id, or `@username`). Everyone else is ignored. Sensitive commands stay with the owner and the account. `.help` / `.الاوامر` is a short section index. `.الاوامر الادارة` explains each command in that section, and `.الاوامر حظر` is one command card. Who may send commands is in `.الاوامر النظام`. `.تحكم` / `.panel` posts an inline button panel in the current chat (categories, command help, plugin on/off, simple settings, and the admins list). `.اللوحة` still opens it and is not shown in the index. The control bot needs inline mode: in BotFather send `/setinline`, choose this bot, and set a placeholder such as `تحكم`. Arabic names are the primary commands, with English aliases. The full list is in [docs/commands.md](docs/commands.md). The Arabic panel guide is [docs/panel-ar.md](docs/panel-ar.md). Who may command an account is in [docs/admins-ar.md](docs/admins-ar.md). See [docs/plugins.md](docs/plugins.md) for how to add a plugin.
...
Revision `0006_account_admins` follows `0005_plan_grants`. It creates `account_admins` when that table is missing and grants the `delegates` plugin to plans that already exist. It does not remove allowlist rows. Run `alembic upgrade head`, then restart the control bot and the worker. The worker must restart so it also hears commands sent by the owner and by account admins. Downgrade drops `account_admins` and removes only the `delegates` allowlist rows.
