...
Userbot commands use `USERBOT_PREFIX` (default `.`). The account itself can send them, and so can the registered owner from their own Telegram account (private chat or a shared group). The owner can add more people with `.رفع ادمن` / `.addadmin` (by reply, numeric id, or `@username`). Everyone else is ignored. Sensitive commands stay with the owner and the account. `.help` / `.الاوامر` lists commands by section and says who may send them. `.الاوامر حظر` shows usage, an example, and who may use that command. `.اللوحة` / `.panel` posts an inline button panel in the current chat (categories, command help, plugin on/off, simple settings, and the admins list). The control bot needs inline mode: in BotFather send `/setinline`, choose this bot, and set a placeholder such as `لوحة`. Arabic names are the primary commands, with English aliases. The full list is in [docs/commands.md](docs/commands.md). The Arabic panel guide is [docs/panel-ar.md](docs/panel-ar.md). Who may command an account is in [docs/admins-ar.md](docs/admins-ar.md). See [docs/plugins.md](docs/plugins.md) for how to add a plugin.
